...
export class HttpHelperMockService {
  private baseUrl: string = environment.mockUrl;
  private httpOptions: {
    headers: HttpHeaders;
    observe: string;
    responseType: string;
  ...
  public request<T = any>(endpoint: string, options: any = {}): Observable<T> {
    ...
    const finalOptions = HttpHelperMockService.createDefaultOptions(options);
    const requestMethod = HttpHelperMockService.getRequestMethod(options);
